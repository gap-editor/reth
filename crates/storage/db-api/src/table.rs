...
`pub trait Compress: Send + Sync + Sized + Debug {`
    ...
    `/// If the type cannot be compressed, return a reference to its underlying bytes if available.`
...
`///`
`/// The [`Table::Key`] type must implement [`Encode`] and [`Decode`],`
`/// while the [`Table::Value`] type must implement [`Compress`] and [`Decompress`].`
`///`
...
`pub trait TableImporter: DbTxMut {`
    ...
    `///`
    `/// This method works with both regular and `DupSort` tables. For `DupSort` tables,`
    `/// all duplicate entries encountered within the range are preserved.`
